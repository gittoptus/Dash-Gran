    ...
    <div className="flex flex-col gap-[20px] w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <Card className="lg:col-span-1 border-border bg-card flex flex-col p-[20px]">
          ...
          <div className="text-[48px] font-bold text-foreground mb-1 leading-none tracking-tight">{((72.9 * (0.85 + 0.15 * Math.max(0.5, filterScale))).toFixed(1)).replace('.', ',')}%</div>
          ...
          <div className="mt-auto pt-4 border-t border-border flex justify-between text-[13px]">
            <div>
              ...
              <div className="font-semibold text-foreground">{Math.round(1455 * filterScale).toLocaleString('pt-BR')} atend.</div>
            </div>
            ...
            <div className="h-[220px] w-full flex-1 min-h-[150px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={channelData.map(d => Object.fromEntries(Object.entries(d).map(([k,v]) => [k, typeof v === "number" ? (v > 100 ? Math.round(v * filterScale) : Number((v * (0.85 + 0.15 * filterScale)).toFixed(1))) : v])))} layout="vertical" margin={{ top: 0, right: 40, left: 10, bottom: 0 }}>
                  ...
                  <Bar dataKey="fcr" radius={[0, 4, 4, 0] as any} barSize={12} background={{ fill: '#f1f5f9', radius: [0,4,4,0] as any }}>
